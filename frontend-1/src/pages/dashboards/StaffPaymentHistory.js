    ...
    <div className="dashboard-container">
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <div className="nav-item active" onClick={() => navigate("/staff-payments")}><img src={payHistoryIcon} className="nav-icon" alt="" /><span>Payment History</span></div>
      ...
      <main className="main-area">
        <header className="top-bar">
          ...
          <div className="top-bar-right">
            <button className="notif-btn" onClick={() => navigate("/staff-notifications")}><img src={bellIcon} className="top-icon" alt="Notif" /></button>
            <div className="user-profile" onClick={() => navigate("/staff-profile")}><img src={userIcon} className="top-avatar" alt="Profile" /></div>
          </div>
        </header>
        ...
        <section className="dashboard-scroll-body">
          ...
          <div className="payment-summary-grid">
            <div className="summary-card">
              <span className="summary-label">Total Revenue (Feb)</span>
              <h3 className="summary-value">â‚±12,450.00</h3>
            </div>
            <div className="summary-card">
              <span className="summary-label">Pending Payments</span>
              <h3 className="summary-value yellow-text">â‚±1,800.00</h3>
            </div>
          </div>
          ...
          <div className="payment-table-container">
            <div className="table-header-flex">
              ...
              <div className="search-wrapper">
                <input type="text" placeholder="Search by Transaction ID or Owner..." className="payment-search-input" />
              </div>
            </div>
            ...
            <table className="payment-table">
              ...
              <tbody>
                {payments.map((p) => (
                  <tr key={p.id}>
                    <td className="txn-id-cell">{p.id}</td>
                    <td>
                    ...
                    <td>{p.service}</td>
                    <td className="amount-bold">{p.amount}</td>
                    <td>{p.date}</td>
                    <td>
                    ...
                    <td><button className="btn-receipt">Receipt</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
